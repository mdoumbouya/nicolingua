...
def main(args):
    ...
    exampleSound = torch.rand(1, 3*16000)
    traced_script_module = torch.jit.script(wav2vec, exampleSound)
    traced_script_module.save("mobile_wawav2vec.pt")


    #exampleFeatures = torch.rand(1, 300, 512)
    #traced_script_module = torch.jit.trace(vaasr_model, exampleFeatures)
    #traced_script_module.save("mobile_vaasr.pt")
