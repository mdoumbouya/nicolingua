...
def dialog_model(current_state, current_language, sorted_class_ids, class_logits):
    ...
    if current_state == 0:
        ...
        possible_asr_classes = [c for c in ASR_CLASSES if c["utt_category"] in possible_utt_categories and c['lang'] == current_language]
    ...
    filtered_sorted_class_probs = torch.softmax(torch.tensor(filtered_sorted_class_logits), dim=0)
    ...
    return class_id, class_prob, new_state_id, new_language, reply_clips